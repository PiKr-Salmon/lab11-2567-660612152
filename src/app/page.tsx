...
export default function RegisterForm() {
  ...
  const [agree, isUserAgreed] = useState(false);
  ...
  const agreementOnChange = (event : React.ChangeEvent<HTMLInputElement>) => {
    isUserAgreed(event.target.checked);
  }
    ...
    <div className="mx-auto vstack gap-3" style={{ width: "400px" }}>
      ...
      <div>
        ...
        <div>
          ...
          <div className={(genderError ? "text-danger": "invalid-feedback")}>Please select gender</div>
        </div>
      </div>
      ...
      <div>
        ...
        {<span className={(buyBottle && buyCap && buyShoes ? "text-success d-block": "invalid-feedback")}>(20% Discounted)</span>}

      </div>

      {/* Terms and conditions */}
      <div>
        <input className="me-2" type="checkbox" onChange={agreementOnChange} checked={agree}/>I agree to the terms and
        conditions
      </div>
